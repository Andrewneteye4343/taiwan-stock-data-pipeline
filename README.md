...
| 指標 | 標準 | 說明 |
|---|---|---|
| EPS（單季） | `fundamental_data.eps` | **由累計 EPS 差值推導**：單季 = 本季累計 − 上季累計；Q1 單季 = 累計 |
| EPS（累計） | `fundamental_data.eps_ytd` | TWSE 損益表原始值（年初至今累計） |
...
| 毛利率 / 營益率 / 淨利率 | 損益表欄位計算 | 有 revenue 等欄位時自動計算（累計/累計，比率不受累計影響） |

> EPS 基準優先序：`eps_ttm` > `eps_ytd` > `eps`。避免「單季 vs 累計」混用造成 PE 不一致。

## 📡 財報資料來源（v2.1 修正）

TWSE 財報開放資料已改為「依產業別分表」，舊端點 `t187ap06_L` / `t187ap07_L` 已停用：
...
管線會抓取全部 6 張表並合併（個別失敗自動跳過）。

**⚠️ 快照累積法（重要）：** 此開放資料只提供「最新一季」快照（無歷史）。因此 TTM EPS 需要**每季自動執行本管線**，把每季累計值存進資料庫，累積四季後 `calculate_ttm_eps` 即可計算歷史。資料不足四季期間，PE 會自動遞補使用累計 EPS（README 遞補鏈）。
...
- **財報開放資料為快照**：TTM 需靠每季排程累積歷史（快照累積法，見「財報資料來源」）
- **`market_index` / `technical_indicators` 表目前無對應 ETL**：schema 保留供未來擴充（技術指標計算函式已存在於 `src/indicators/technical.py`，尚未寫入 DB）
- 掃描型/圖片 PDF 不在本專案範圍
